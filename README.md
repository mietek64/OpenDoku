...
```powershell
py -3.13 -m PyInstaller --onefile --windowed --name "OpenDoku" `
  --add-data "index.html;." `
  --add-data "style.css;." `
  --add-data "script.js;." `
  --icon "icon.ico" `
  ...
  --hidden-import webview.menu `
  --hidden-import webview.event `
  --collect-all webview `
...
```bash
python3.13 -m PyInstaller --onefile --windowed --name "OpenDoku" \
  --add-data "index.html:." \
  --add-data "style.css:." \
  --add-data "script.js:." \
  --icon "icon.png" \
...
```bash
python3.13 -m PyInstaller --onefile --name "OpenDoku" \
  --add-data "index.html:." \
  --add-data "style.css:." \
  --add-data "script.js:." \
  --icon "icon.png" \
...
> `--windowed` is omitted on Linux — it has no effect there.
